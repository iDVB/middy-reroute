...
const rerouteMiddleware = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (opts = {}, handler, next) {
    ...
    try {
      ...
      const keyExists = yield doesKeyExist(parsedURI.pathname); // Detect if needing friendly URLs
      ...
      if (isUnFriendlyUrl && (!keyExists || isIndex)) {
        ...
        if (match) {
          ...
          event = isRedirectURI(match.status) ? redirect(`${match.parsedTo}${parsedURI.search}`, match.status) : isAbsoluteURI(match.parsedTo) ? yield proxy(match.parsedTo, handler.event) : yield rewrite(forceDefaultDoc(match.parsedTo), s3DomainName, handler.event);
        } else {
...
const forceDefaultDoc = uri => {
  if (path__default['default'].extname(uri) === '' && !!options.defaultDoc) {
    ...
    newURL.pathname = path__default['default'].join(newURL.pathname, options.defaultDoc);
...
const rewrite = /*#__PURE__*/function () {
  var _ref4 = _asyncToGenerator(function* (to, host, event) {
    ...
    const keyDoesNotExist = !(yield doesKeyExist(to));
    const has404 = yield get404Response();
