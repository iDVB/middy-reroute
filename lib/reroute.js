...
const findMatch = (data, path$$1, host, protocol) => {
  ...
  const match = _find(data, o => {
    ...
    params = isAbsoluteURI(o.from) ? from(fullUri) : from(url.parse(path$$1).pathname); // If there specific language rules, do they match
...
function () {
  var _ref = _asyncToGenerator(function* (opts = {}, handler, next) {
    ...
    const defaults = {
      functionSuffix,
      tableSuffix,
      tableName: getTableFromFunction(context.functionName, functionSuffix, tableSuffix)
    };
    ...
    `);
      ...
      handler.event = !!domainData ? deepmerge(handler.event, test) : handler.event;
    } catch (err) {
      log('Throwing Error for main thread');
      throw err;
    }
...
const getHeaderValues$1 = (paramArr, headers) => paramArr.map(param => headers[param] && headers[param][0] && headers[param][0].value); // from:  us-east-1.marketing-stack-proxy-prod-viewerRequest
