...
const userAgentParser = (testuserAgentArray, userAgentHeader) => {
  const _UAParser$browser = UAParser(userAgentHeader).browser,
...
const findMatch = (data, path, host, protocol) => {
  ...
  const match = _find(data, o => {
    ...
    const agentPass = !!o.conditions.useragent ? !!options.userAgent && userAgentParser(o.conditions.useragent, options.userAgent) : false;
    console.log({
      agentPass,
      countryPass
