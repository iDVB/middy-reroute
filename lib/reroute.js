...
const _excluded = ["arrayMerge"],
      _excluded2 = ["arrayMerge"];
...
const rerouteMiddleware = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (opts = {}, handler, next) {
    var _origin$s, _origin$s2, _s3DomainName$split;

    const request = handler.event.Records[0].cf.request;
    const origin = request.origin;

    const _getHeaderValues = getHeaderValues$1(['host', 'cloudfront-viewer-country', 'accept-language', 'user-agent'], request.headers),
    ...
    const defaults = {
      ...
      regex: {
        ...
        ruleline: /([^\s\r\n]+)(?:\s+)([^\s\r\n]+)(?:\s+(\d+)([!]?))?(?:(?:\s+)?([^\s\r\n]+))?/,
        absoluteUri: /^(?:[a-z]+:)?\/\//,
        bucketNameFromDomain: /(^.+)(\.s3(?:\.[a-z]+-[a-z]+-[0-9]+)?\.amazonaws\.com$)/
      },
      defaultStatus: 301,
    ...
    const s3DomainName = origin === null || origin === void 0 ? void 0 : (_origin$s = origin.s3) === null || _origin$s === void 0 ? void 0 : _origin$s.domainName;
    const originPath = origin === null || origin === void 0 ? void 0 : (_origin$s2 = origin.s3) === null || _origin$s2 === void 0 ? void 0 : _origin$s2.path;
    const originBucket = s3DomainName === null || s3DomainName === void 0 ? void 0 : (_s3DomainName$split = s3DomainName.split(defaults.regex.bucketNameFromDomain)) === null || _s3DomainName$split === void 0 ? void 0 : _s3DomainName$split[1];
    ...
    `);
    console.log(util__namespace.inspect({
      middyReroute: request
    }, {
      depth: null
    })); // If "X-No-Middy-Reroute" header is present then do no run

    if (!!request.headers['X-No-Middy-Reroute'] === true) return;
    console.log('after return', request.headers['X-No-Middy-Reroute']); // Bypass if Origin is not S3

    if (!s3DomainName) return;

    try {
      // Check if file exists
      const parsedURI = new URL(`https://example.com${request.uri}?${request.querystring}`);
      const keyExists = yield doesKeyExist({
        key: parsedURI.pathname,
        originPath
      }); // Detect if needing friendly URLs

      const isUnFriendlyUrl = options.friendlyUrls && parsedURI.pathname.match(options.regex.htmlEnd);
      ...
      if (isUnFriendlyUrl && (!keyExists || isIndex)) {
        ...
        if (match) {
          ...
          event = isRedirectURI(match.status) ? redirect(`${match.parsedTo}${parsedURI.search}`, match.status) : isAbsoluteURI(match.parsedTo) ? yield proxy(match.parsedTo, handler.event) : yield rewrite({
...
const getHeaderValues$1 = (paramArr, headers) => paramArr.map(param => headers[param] && headers[param][0] && headers[param][0].value);
...
const forceDefaultDoc = uri => {
  if (path__default["default"].extname(uri) === '' && !!options.defaultDoc) {
    const newURL = new URL(`https://example.com/${uri}`);
    const pathname = path__default["default"].join(newURL.pathname, options.defaultDoc);
...
const findMatch = (data, path, host, protocol) => {
  ...
  const match = _find__default["default"](data, o => {
    ...
    params = isAbsoluteURI(o.from) ? from(fullUri) : from(parsedPath.pathname); // If there specific language rules, do they match
  ...
  return match && _objectSpread2(_objectSpread2({}, match), {}, {
    parsedTo: `${replaceAll(params, match.to)}${parsedPath.search || ''}${parsedPath.hash || ''}`
  });
}; ///////////////////////
// Data Fetching     //
///////////////////////
...
const doesKeyExist = ({
  ...
  const Key = path__default["default"].join(originPath, rawKey).replace(/^\/+([^\/])/, '$1');
  log('doesKeyExist: ', {
    Key,
    Bucket: options.originBucket
  });
  const cacheKey = `doesKeyExist_${options.rulesBucket}_${Key}`;
  return cache$1.get(cacheKey, () => S3.headObject({
    Bucket: options.originBucket,
    Key
  }).promise().then(data => {
...
const getRedirectData = ({
  ...
  const file = !options.multiFile ? options.file : `${options.file}_${options.host}`;
  const Key = path__default["default"].join(originPath, file).replace(/^\/+([^\/])/, '$1');
...
const get404Response = ({
  ...
  const Key = path__default["default"].join(originPath, options.custom404).replace(/^\/+([^\/])/, '$1');
  const cacheKey = `get404Response_${options.rulesBucket}_${Key}`;
  return cache$1.get(cacheKey, () => S3.getObject({
    Bucket: options.originBucket,
    Key
  }).promise().then(({
    Body
...
const rewrite = /*#__PURE__*/function () {
  var _ref4 = _asyncToGenerator(function* ({
    ...
    log('Rewriting: ', to);
    const resp = !isAbsoluteURI(to) && !(yield doesKeyExist({
      key: to,
      originPath
    })) && (yield get404Response({
      originPath
    })) || deepmerge(event, {
      Records: [{
        cf: {
...
const rerouteOrigin = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (opts = {}, handler, next) {
    ...
    s3DomainName && s3DomainName.replace(S3_SUFFIX, '');
...
const getHeaderValues = (paramArr, headers) => paramArr.map(param => headers[param] && headers[param][0] && headers[param][0].value); // from:  us-east-1.myproject-prod-originrequest
