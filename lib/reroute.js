...
const all = (arr, _ref2 = {}) => {
  ...
  return merge__default['default'].all(
    arr,
    _objectSpread2(
      _objectSpread2({}, rest),
      {},
...
const clone = (value, options) =>
  merge__default['default'](emptyTarget(value), value, options);
...
const rerouteMiddleware = /*#__PURE__*/ (function () {
  var _ref = _asyncToGenerator(function* (opts = {}, handler, next) {
    ...
    const _getHeaderValues = getHeaderValues$1(
    ...
    const s3DomainName =
      origin === null || origin === void 0
        ? void 0
        : (_origin$s = origin.s3) === null || _origin$s === void 0
        ? void 0
        : _origin$s.domainName;
    const originPath =
      origin === null || origin === void 0
        ? void 0
        : (_origin$s2 = origin.s3) === null || _origin$s2 === void 0
        ? void 0
        : _origin$s2.path;
    const originBucket =
      s3DomainName === null || s3DomainName === void 0
        ? void 0
        : (_s3DomainName$split = s3DomainName.split(
            defaults.regex.bucketNameFromDomain,
          )) === null || _s3DomainName$split === void 0
        ? void 0
        : _s3DomainName$split[1];
    options = all([
      defaults,
      {
      ...
      if (isUnFriendlyUrl && (!keyExists || isIndex)) {
        ...
        if (match) {
          ...
          event = isRedirectURI(match.status)
            ...
            ? yield proxy(match.parsedTo, handler.event)
            : yield rewrite({
...
const getHeaderValues$1 = (paramArr, headers) =>
  paramArr.map(
...
const forceDefaultDoc = (uri) => {
  if (path__default['default'].extname(uri) === '' && !!options.defaultDoc) {
    ...
    const pathname = path__default['default'].join(
      newURL.pathname,
      options.defaultDoc,
    );
...
const lambdaReponseToObj = (req) => {
  ...
  return {
    method,
    headers: _omit__default['default'](
      _reduce__default['default'](
        req.headers,
        (result, value, key) =>
          _objectSpread2(
...
const userAgentParser = (testuserAgentArray, userAgentHeader) => {
  const _UAParser$browser =
      UAParser__default['default'](userAgentHeader).browser,
  ...
  const match = _find__default['default'](testuserAgentArray, (uaRule) => {
    ...
    const cleanVersion = semver__default['default'].valid(
      semver__default['default'].coerce(version),
    );
    const isVersion = semver__default['default'].satisfies(
      cleanVersion,
      versionRule,
    );
...
const findMatch = (data, path, host, protocol) => {
  ...
  const match = _find__default['default'](data, (o) => {
    ...
    const languagePass = !!o.conditions.language
      ? !!options.language &&
        !!langParser__default['default'].pick(
          o.conditions.language,
          options.language,
          {
            loose: true,
          },
    ...
    const countryPass = !!o.conditions.country
      ...
      : true; // If there specific user-agent rules, do they match

    const agentPass = !!o.conditions.useragent
      ? !!options.userAgent &&
        userAgentParser(o.conditions.useragent, options.userAgent)
      : true; // Let's make sure all our conditions pass IF set
...
const doesKeyExist = ({ key: rawKey, originPath }) => {
  const Key = path__default['default']
    .join(originPath, rawKey)
    .replace(/^\/+([^\/])/, '$1');
  log('doesKeyExist: ', {
    Key,
...
const getProxyResponse = (resp) => {
  ...
  const headers = _omitBy__default['default'](
    _reduce__default['default'](
      resp.headers,
      (result, value, key) =>
        _objectSpread2(
...
const rewrite = /*#__PURE__*/ (function () {
  var _ref4 = _asyncToGenerator(function* ({ to, host, event, originPath }) {
    log('Rewriting: ', to);
    const resp =
      (!isAbsoluteURI(to) &&
        !(yield doesKeyExist({
...
const proxy = (url, event) => {
  ...
  const config = _objectSpread2(
    _objectSpread2({}, lambdaReponseToObj(request)),
    {},
    {
      url,
      validateStatus: null,
  ...
  return axios__default['default'](config)
    .then((resp) => {
      log(
        'PROXY data: ',
        _omit__default['default'](resp, ['request', 'config']),
      );
...
const rerouteOrigin = /*#__PURE__*/ (function () {
  var _ref = _asyncToGenerator(function* (opts = {}, handler, next) {
    ...
    DDB =
      DDB ||
      new AWS__default['default'].DynamoDB({
    ...
    try {
      ...
      handler.event = !!domainData
        ? dotProp__default['default'].merge(handler.event, ORIGIN_S3_DOTPATH, {
