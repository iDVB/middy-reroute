...
const rerouteMiddleware = async (opts = {}, handler, next) => {
  ...
  const [host, country, language, userAgent] = getHeaderValues$1(['host', 'cloudfront-viewer-country', 'accept-language', 'user-agent'], request.headers);
  const defaults = {
    file: '_redirects',
    rules: undefined,
    multiFile: false,
  ...
  try {
    ...
    if (isUnFriendlyUrl && (!keyExists || isIndex)) {
      ...
      if (match) {
        ...
        event = isRedirectURI(match.status) ? redirect(`${match.parsedTo}${parsedURI.search}`, match.status) : isAbsoluteURI(match.parsedTo) ? await proxy(match.parsedTo, handler.event) : await rewrite({
...
const get404Response = ({
  ...
  const Key = path.join(originPath, options.custom404).replace(/^\/+([^\/])/, '$1');
...
const rewrite = async ({
  ...
  log('Rewriting: ', to);
  const resp = !isAbsoluteURI(to) && !(await doesKeyExist({
    key: to,
    originPath
