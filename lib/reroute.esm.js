...
const deepmerge = (x, y, _ref = {}) => {
  ...
  return merge(x, y, _objectSpread2({}, rest, {
...
const all = (arr, _ref2 = {}) => {
  ...
  return merge.all(arr, _objectSpread2({}, rest, {
    arrayMerge: combineMerge
  }));
};
...
const lambdaReponseToObj = req => {
  ...
  return {
    method,
    headers: _omit(_reduce(req.headers, (result, value, key) => _objectSpread2({}, result, {
...
const findMatch = (data, path, host, protocol) => {
  ...
  return match && _objectSpread2({}, match, {
...
const getProxyResponse = resp => {
  ...
  const headers = _omitBy(_reduce(resp.headers, (result, value, key) => _objectSpread2({}, result, {
    [key]: [{
...
const proxy = (url, event) => {
  ...
  const config = _objectSpread2({}, lambdaReponseToObj(request), {
    url,
    validateStatus: null,
