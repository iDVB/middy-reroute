...
const rerouteMiddleware = async (opts = {}, handler, next) => {
  ...
  try {
    ...
    const keyExists = await doesKeyExist(parsedURI.pathname); // Detect if needing friendly URLs
    ...
    if (isUnFriendlyUrl && (!keyExists || isIndex)) {
      ...
      if (match) {
        ...
        event = isRedirectURI(match.status) ? redirect(`${match.parsedTo}${parsedURI.search}`, match.status) : isAbsoluteURI(match.parsedTo) ? await proxy(match.parsedTo, handler.event) : await rewrite(forceDefaultDoc(match.parsedTo), s3DomainName, handler.event);
      } else {
